	...
	*/
	...
	class thebestvideo_widget extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			$rss_channel_box = apply_filters( 'widget_rss_channel', $instance['rss_channel'] );
			$rss_channel_array = explode("\r\n", $rss_channel_box);
			...
			foreach ($rss_channel_array as $rss_channel) {
				...
				if( !is_wp_error( $rss ) ){
					...
					if( $maxitems == 0 ) {}	// Nothing
						...
						foreach ( $rss_items as $item ){
							...
							if($item_first_img) {
								$str_item = '<a target="_blank" title="The Best Video: ' .$item_title .'" href="' .$item_link .'" rel="nofollow"><img src="'.$item_first_img.'" alt="'.$item->get_title().'" /></a>';
								
								if ($enable_post_titles) {
									$str_description = '<div class="tbv_description">' .$item_title .'</div>';
								}
								else {
									$str_description = '</br></br>';
								}

								$arr_rss_full[] = $str_item . '|' .$str_description ;
							}
						}
					}
				}
			...
			// Clearing. Remove dublicates.
			$arr_rss_unique = array_unique($arr_rss_full);
			$randkeys = array_rand($arr_rss_unique, $maxposts);
			
for ($i = 0; $i < $maxposts; $i++) {
				$item = $arr_rss_unique[$randkeys[$i]];
				$rss_item = explode("|", $item);
				echo $rss_item [0];
				echo $rss_item [1];	
}
		...
		public function form( $instance ) { // Admin console
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'rss_channel' ); ?>"><?php _e( 'RSS Channel (One per Line):' ); ?></label> 			
				<textarea class="widefat" id="<?php echo $this->get_field_id( 'rss_channel' ); ?>" name="<?php echo $this->get_field_name( 'rss_channel' ); ?>" rows="5"><?php echo esc_attr( $rss_channel ); ?></textarea>
			</p>
